...
namespace StudentExercises {
    class Program {
        static void Main (string[] args) {
            ...
            var studentwithMostExerc = (from s in students select new {
            ...
            IEnumerable<Exercise> exercises5 = db.Query<Exercise> (@"Select * from Exercise");
            foreach (Exercise ex in exercises5) {
                Console.WriteLine ($"{ex.Name} and {ex.Language}did this work?");
            }

            Console.WriteLine ("----------------------------------------");

            //4. Find all the exercises in the database where the language is JavaScript
            IEnumerable<Exercise> exercises6 = db.Query<Exercise> (@"Select * from Exercise where Language = 'Javascript';");
            foreach (Exercise exer in exercises6) {
                Console.WriteLine ($"{exer.Name} are used in Javascript");
            }
            //5. Insert a new exercise into the database.
            // db.Execute(@"INSERT INTO Exercise (Name, Language) VALUES ('StudentExercise5', 'Sql');");

            //6. Find all instructors in the database. Include each instructor's cohort.
        //    List<Instructor> instructors5 = db.Query<Instructor> (@"Select * from Instructor").ToList();
        //     List<Cohort> cohort5 = db.Query<Cohort>(@"Select * from Cohort").ToList();
        //     foreach (Instructor inst in instructors5) {
        //         Console.WriteLine ($"{inst.FirstName} {inst.LastName} teaches {cohort5.Find(c=>c.Id == inst.CohortId).Name}");
        //     }

            List<Instructor> instructorList1 =db.Query<Instructor, Cohort, Instructor>(@"
                SELECT 
                    i.FirstName,
                    i.LastName,
                    i.CohortId,
            ...
            ", (instructor6, cohort6) =>
            {
                instructor6.Cohort = cohort6;
                return instructor6;
            }).ToList();
            foreach(Instructor inst in instructorList1)
            {
                Console.WriteLine($"{inst.FirstName} {inst.LastName} teaches {inst.Cohort.Name}");
            }
